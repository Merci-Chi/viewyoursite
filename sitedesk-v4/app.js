const KEY = "sitedesk.v4";
const TOKEN_KEY = "sitedesk.githubToken";
const SESSION_KEY = "sitedesk.v4.session";
...
const SKIP_DIRS = new Set([
  ...
  "sitedesk", "sitedesk-v2", "sitedesk-v3", "sitedesk-v4", "sitedesk-data",
]);
...
function shell(user, inner) {
  ...
  return `
    <div class="app-shell">
      <aside class="sidebar">
        <div class="brand"><b>SiteDesk</b><span class="muted">v4 · ${esc(user.role)}</span></div>
...
function page(user) {
  ...
  if (route.name === "sites" && (user.role === "admin" || user.role === "builder")) return sitesPage(user);
...
function stepperHtml(lead, tab) {
  ...
  const current = ["call", "brief", "build", "live"].includes(tab) ? tab : path.step;
  const steps = [
  ...
  const next = tab === "live" && path.step === "live"
    ? "Next: send to owner."
    : (tab === "manage" ? "Next: open a site to tiny-edit or republish." : path.next);
  return `
...
function leadPage(user, id) {
  ...
  if (!["call", "brief", "build", "live", "messages"].includes(tab)) tab = path.step;
