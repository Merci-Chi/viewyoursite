  ...
  return `
    <div class="app-shell ${navOpen ? "nav-open" : ""}">
      <header class="bar">
        ...
        <b>SiteDesk</b>
        <span class="muted">${esc(user.role)}</span>
      </header>
...
function settingsPage(user) {
  ...
  return `
    ...
    ${user && user.role === "admin" ? `
      <p class="section-label">Queued</p>
      <p class="muted">${queued.length}</p>
      ${queued.length ? `
        <table class="table">
          <thead><tr><th>Business</th><th>Slug</th><th></th></tr></thead>
          <tbody>
            ${queued.map((l) => `<tr>
        ...
        <div class="row" style="margin-top:12px">
          <button class="btn primary" type="button" id="publish-queued" ${busy ? "disabled" : ""}>Publish queued</button>
        </div>
      ` : ""}
    ` : ""}`;
}
